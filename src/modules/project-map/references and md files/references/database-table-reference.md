...
create table public.proj_s_payment_method (
  id bigint generated by default as identity not null,
  method_name character varying null default '100'::character varying,
  created_at timestamp with time zone not null default now(),
  method_description text null,
  constraint proj_s_payment_method_pkey primary key (id)
) TABLESPACE pg_default;



create table public.proj_t_paid_sheet (
  id serial not null,
  run_id integer not null,
  phone_number text null,
  dot_number text null,
  state_route text null,
  extra_notes text null,
  is_paid boolean not null default false,
  paid_date date null,
  paid_reference text null,
  installer_signature_date date null,
  psb_representative_name text null,
  psb_representative_date date null,
  created_at timestamp with time zone not null default now(),
  updated_at timestamp with time zone not null default now(),
  created_by integer null,
  updated_by integer null,
  constraint proj_t_paid_sheet_pkey primary key (id),
  constraint proj_t_paid_sheet_run_id_key unique (run_id),
  constraint proj_t_paid_sheet_run_id_fkey foreign KEY (run_id) references proj_t_runs (id) on delete CASCADE
) TABLESPACE pg_default;

create index IF not exists idx_proj_t_paid_sheet_run_id on public.proj_t_paid_sheet using btree (run_id) TABLESPACE pg_default;
